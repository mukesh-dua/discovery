...
The agent and tool are published as:
...
```bash
# Rebuild image (if Dockerfile or cp2k_utils.py changed)
az acr build --registry {registry_name} --image cp2k:v19 --platform linux/amd64 <bundle-dir>

# Republish local tool + agent
discovery catalog_publish_agent --agent_key cp2k
```
