...
REQUIRED_RPS=(
    Microsoft.Network Microsoft.Compute Microsoft.Storage Microsoft.ManagedIdentity Microsoft.AlertsManagement
...
for RP in "${REQUIRED_RPS[@]}"; do
    ...
    if [[ "$STATUS" != "Registered" ]]; then
        issues+=("Resource Provider $RP is not registered.")
        UNREGISTERED_RPS+=("$RP")
    fi
done
...
for ROLE in "${REQUIRED_ROLES[@]}"; do
    ASSIGNED=$(az role assignment list --assignee "$USER_ID" --query "[?roleDefinitionName=='$ROLE']" -o tsv)
    if [[ -z "$ASSIGNED" ]]; then
        echo "[WARN] $USER_ID does not have $ROLE role."
        issues+=("User $USER_ID does not have $ROLE role.")
        MISSING_ROLES+=("$ROLE")
    else
...
fetch_quota() {
    ...
    if [[ -z "$RESPONSE" || "$RESPONSE" == "{}" ]]; then
        ...
        issues+=("No quota response for location $LOCATION.")
        return
    fi
    echo "[INFO] AI Foundry TPM quota details for location $LOCATION:"
    echo "============================="
    echo "$RESPONSE" | jq -c '.value[]' | while read -r item; do
        MODEL=$(echo "$item" | jq -r '.name.value')
        CURRENT_TPM=$(echo "$item" | jq -r '.currentValue')
        LIMIT_TPM=$(echo "$item" | jq -r '.limit')
        UNIT=$(echo "$item" | jq -r '.unit')
        # Convert limit to TPM (API returns in thousands)
        TPM_LIMIT=$((LIMIT_TPM * 1000))
        # Model matching logic for Standard and GlobalStandard variants
        if [[ "$MODEL" == "OpenAI.Standard.gpt-4o" || "$MODEL" == "OpenAI.GlobalStandard.gpt-4o" ]]; then
            RPM=$(awk "BEGIN {print $TPM_LIMIT / (1000/6)}")
            echo "[INFO] Model: $MODEL ($LOCATION)"
            echo "[INFO] Current TPM Usage: $CURRENT_TPM"
            echo "[INFO] TPM Limit: $TPM_LIMIT"
            echo "[INFO] Estimated RPM: $RPM"
            AVAILABLE_TPM=$(awk "BEGIN {print $TPM_LIMIT - $CURRENT_TPM}")
            echo "[INFO] Available TPM: $AVAILABLE_TPM"
            # Threshold checks
            if [[ -z "$AVAILABLE_TPM" || "$AVAILABLE_TPM" -lt 250000 ]]; then
                echo "[WARN] TPM quota for GPT-4o is less than 250K in $LOCATION."
                issues+=("TPM quota for GPT-4o is less than 250K in $LOCATION.")
            else
                echo "[INFO] Sufficient TPM quota ($AVAILABLE_TPM) for $MODEL in $LOCATION."
            fi
            echo "-----------------------------"
        fi
        if [[ "$MODEL" == "OpenAI.Standard.text-embedding-3-small" || "$MODEL" == "OpenAI.GlobalStandard.text-embedding-3-small" ]]; then
            RPM=$(awk "BEGIN {print $TPM_LIMIT / 1000}")
            echo "[INFO] Model: $MODEL ($LOCATION)"
            echo "[INFO] Current TPM Usage: $CURRENT_TPM"
            echo "[INFO] TPM Limit: $TPM_LIMIT"
            echo "[INFO] Estimated RPM: $RPM"
            ...
            echo "[INFO] Available TPM: $AVAILABLE_TPM"
            # Threshold checks
            if [[ -z "$AVAILABLE_TPM" || "$AVAILABLE_TPM" -lt 600000 ]]; then
                echo "[WARN] TPM quota for Text-Embedding-3-Small is less than 600K in $LOCATION."
                issues+=("TPM quota for Text-Embedding-3-Small is less than 600K in $LOCATION.")
            else
                echo "[INFO] Sufficient TPM quota ($AVAILABLE_TPM) for $MODEL in $LOCATION."
            fi
            echo "-----------------------------"
...
if [[ -z "$VCPU_QUOTA" || "$AVAILABLE_VCPU" -lt 12 ]]; then
    ...
    issues+=("Insufficient available vCPU quota for Standard_D4s_v6 SKU (need at least 12) in $REGION.")
else
...
if [[ "$AVAILABLE_QUOTA" -lt 4096 ]]; then
    ...
    issues+=("NetApp available quota less than 4TB in $REGION for account $ACCOUNT_NAME.")
fi
...
if [[ ${#issues[@]} -eq 0 ]]; then
    ...
    if [[ ${#UNREGISTERED_RPS[@]} -gt 0 ]]; then
        ...
        az role assignment list --assignee "$USER_ID" --query "[?roleDefinitionName=='Owner' || roleDefinitionName=='User Access Administrator' || roleDefinitionName=='Contributor']" -o tsv | grep -E 'Owner|User Access Administrator|Contributor' > /dev/null
        if [[ $? -eq 0 ]]; then
    ...
    if [[ ${#MISSING_ROLES[@]} -gt 0 ]]; then
        ...
        az role assignment list --assignee "$USER_ID" --query "[?roleDefinitionName=='Owner' || roleDefinitionName=='User Access Administrator' || roleDefinitionName=='Role Based Access Control Administrator']" -o tsv | grep -E 'Owner|User Access Administrator|Role Based Access Control Administrator' > /dev/null
        if [[ $? -eq 0 ]]; then
            read -p "Do you want to assign ALL missing roles now? (y/n): " ASSIGN_ALL
            if [[ "$ASSIGN_ALL" == "y" ]]; then
                for ROLE in "${MISSING_ROLES[@]}"; do
                    echo "[INFO] Assigning $ROLE to $USER_ID..."
                    az role assignment create --assignee "$USER_ID" --role "$ROLE"
                done
