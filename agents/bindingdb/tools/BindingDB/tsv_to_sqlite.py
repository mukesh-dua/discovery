...
class BindingDBConverter:
    ...
    # Base URL for BindingDB downloads
    _DOWNLOADS_BASE = "https://www.bindingdb.org/rwd/bind/downloads/"
    # Download page to scrape for latest TSV filename
    _DOWNLOAD_PAGE = "https://www.bindingdb.org/rwd/bind/chemsearch/marvin/Download.jsp"
    ...
    @classmethod
    def discover_latest_tsv_url(cls) -> Optional[str]:
        """Scrape the BindingDB download page to find the latest All TSV URL."""
        import re
        try:
            resp = requests.get(cls._DOWNLOAD_PAGE, timeout=30)
            resp.raise_for_status()
            # Match filenames like BindingDB_All_YYYYMM_tsv.zip
            matches = re.findall(r'BindingDB_All_(\d{6})_tsv\.zip', resp.text)
            if not matches:
                return None
            latest = max(matches)  # lexicographic max = most recent YYYYMM
            return f"{cls._DOWNLOADS_BASE}BindingDB_All_{latest}_tsv.zip"
        except Exception as e:
            print(f"⚠️  Auto-discovery of latest TSV URL failed: {e}")
            return None
    ...
    def __init__(self, output_db: str = "bindingdb.db", tsv_url: Optional[str] = None):
        ...
        Args:
            ...
            tsv_url: Custom download URL (auto-discovered from BindingDB if not provided)
        """
        self.output_db = output_db
        if tsv_url:
            self.tsv_url = tsv_url
        else:
            discovered = self.discover_latest_tsv_url()
            if discovered:
                print(f"🔍 Auto-discovered latest TSV URL: {discovered}")
                self.tsv_url = discovered
            else:
                print(f"⚠️  Using fallback URL: {self.DEFAULT_TSV_URL}")
