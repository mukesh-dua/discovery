...
This agent operates as a `kind: prompt` agent within Discovery Studio.
    ...
    User Input → BindingDB (LLM) → BindingDB Tool (Container) → Results

- **Model:** Configured via the `{{model}}` parameter at deploy time
- **Tool:** BindingDB container with local SQLite database for binding affinity queries
...
|---|---|---|
| `bindingDB` | `tools/BindingDB/` | BindingDB is a tool for accessing detailed binding affinity and thermodynamic data from the BindingDB database, specializing in Kd, Ki, IC50 measur... |
